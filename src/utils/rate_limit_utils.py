...
class RateLimitError(Exception):
    ...
    def __init__(self, message: str, retry_after: int = 60, original_exception: Exception = None):
...
def is_rate_limit_error(exception: Exception) -> bool:
    ...
    if any(keyword in exception_name for keyword in ['ratelimit', 'rate_limit', 'toomanyrequests']):
        return True
...
def extract_retry_after_seconds(exception: Exception) -> int:
    ...
    retry_after_match = re.search(r'retry[_-]?after[:\s]*(\d+)', error_message, re.IGNORECASE)
    ...
    try_again_match = re.search(r'try[:\s]*again[:\s]*in[:\s]*(\d+)[:\s]*seconds?', error_message, re.IGNORECASE)
    if try_again_match:
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    print(f"Rate limit error in {func.__name__}, retrying in {delay:.1f} seconds (attempt {attempt + 1}/{max_retries + 1})")
                    time.sleep(delay)
