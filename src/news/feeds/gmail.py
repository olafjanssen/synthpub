...
CREDENTIALS_FILE = Path(
    "../creds/client_secret_675383632896-e6khlbtubtdo80f9v877kkqkore1mp7e.apps.googleusercontent.com.json"
).resolve()
...
def fetch_gmail_content(max_results: int = 50) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        error(
            "GMAIL", "Content fetch failed", f"Error fetching Gmail content: {str(e)}"
        )
        return []
    ...
    def fetch_content(url: str) -> List[Dict[str, Any]]:
        try:
            ...
            error(
                ...
                f"Error fetching Gmail content: {str(e)}",
            )
            return []
