...
def load_settings():
    ...
    try:
        if os.path.exists("settings.yaml"):
            with open("settings.yaml", "r") as f:
                ...
                update_interval = scheduler_settings.get(
                    "update_interval_minutes", DEFAULT_UPDATE_INTERVAL_MINUTES
                )
                threshold_hours = scheduler_settings.get(
                    "update_threshold_hours", DEFAULT_TOPIC_UPDATE_THRESHOLD_HOURS
                )
    ...
    return (
        ...
        DEFAULT_TOPIC_UPDATE_THRESHOLD_HOURS,
    ...
    while not _stop_event.is_set():
        try:
            ...
            if enabled:
                info(
                    "SCHEDULER",
                    f"Running update check (interval: {update_interval}m, threshold: {threshold_hours}h)",
                )
                check_and_update_topics()
            else:
                info(
                    "SCHEDULER",
                    "Scheduler is disabled in settings, skipping update check",
                )
