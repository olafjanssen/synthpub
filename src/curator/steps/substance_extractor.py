...
class SubstanceResponse(BaseModel):
    ...
    new_information: str = Field(
        description="New information from the article, leave empty if there is no new information"
    )
    enforcing_information: str = Field(
        description="Enforcing information from the article, leave empty if there is no enforcing information"
    )
    contradicting_information: str = Field(
        description="Contradicting information from the article, leave empty if there is no contradicting information"
    )
...
def process(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        new_state["contradicting_information"] = (
            extracted_substance.contradicting_information
        )
...
def extract_substance(
    topic: Topic, current_article: Article, feed_content: str, feed_item: FeedItem
) -> SubstanceResponse:
    """
    Extract substance from new content compared to existing article.
