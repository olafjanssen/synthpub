        ...
        for root_id, versions in article_versions.items():
            ...
            for article in versions:
                try:
                    ...
                    print(
                        f"  Migrated article: {article.title} (version {article.version})"
                    )
                except ValueError as e:
                    print(f"  Error: {e}")
...
def main():
    ...
    print("\nMigration complete!")
    print(
        "To use the hierarchical DB structure, set the DB_IMPLEMENTATION environment variable to 'hierarchical'"
    )
