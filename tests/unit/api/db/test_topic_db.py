...
def test_invalidate_cache():
    """Test invalidating the topic cache."""
...
def test_ensure_cache():
    ...
    with patch("api.db.topic_db._cache_initialized", False):
        with patch("api.db.topic_db._load_all_topics_from_disk", return_value=mock_topics_list):
...
def test_load_all_topics_from_disk(mock_topic):
    ...
    with patch("api.db.topic_db.ensure_db_exists"):
        with patch("api.db.topic_db.db_path", return_value=Path("/mock/db/topics")):
            with patch("pathlib.Path.glob", return_value=[Path("/mock/db/topics/test-topic.yaml")]):
            ...
            with patch("pathlib.Path.exists", return_value=True):
                with patch("os.rename") as mock_rename:
                    with patch("os.path.isdir", return_value=False):
                        ...
                        with patch("api.db.article_db.mark_article_deleted", return_value=True):
                            ...
                            with patch.dict("sys.modules", {"api.db.project_db": mock_project_db}):
