        ...
        with patch("src.api.db.project_db.ensure_path_exists"):
            with patch("builtins.open", mock_open()) as mock_file:
                with patch("src.api.db.project_db.add_to_entity_cache"):
                    with patch(
                        "src.api.db.project_db.create_slug", return_value="test-project"
                    ):
                        # Setup mock path
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("builtins.open", mock_open()):
                with patch(
                    ...
                    project_dir1.__truediv__.return_value = Path(
                        "/mock/vault/project-1/metadata.yaml"
                    )
                    project_dir2 = MagicMock(spec=Path)
                    project_dir2.__truediv__.return_value = Path(
                        "/mock/vault/project-2/metadata.yaml"
                    )
