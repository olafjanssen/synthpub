...
def test_sanitize_filename():
    ...
    assert _sanitize_filename("http://example.com/article") == "http_example.com_article"
    
    # Test with special characters
    assert _sanitize_filename("http://example.com/article!@#$%^&*()_+") == "http_example.com_article____________"
    
    # Test with spaces
    assert _sanitize_filename("http://example.com/some article") == "http_example.com_some_article"

@pytest.mark.skip(reason="Cache manager implementation has changed significantly")
def test_get_cache_path():
    """Test getting the cache path for a URL."""
    with patch("api.db.cache_manager._sanitize_filename", return_value="sanitized_filename"):
...
def test_initialize_cache(mock_cache_dir):
    ...
    mock_files = [
        mock_cache_dir / "file1.json",
        mock_cache_dir / "file2.json"
    ]
    ...
    with patch("api.db.cache_manager.CACHE_PATH", return_value=mock_cache_dir):
        with patch("api.db.cache_manager.ensure_cache_exists"):
            with patch("pathlib.Path.glob", return_value=mock_files):
                with patch("api.db.cache_manager._process_cache_file", side_effect=[mock_metadata1, mock_metadata2]):
                    # Reset _cache_initialized to False for the test
                    with patch("api.db.cache_manager._cache_initialized", False):
                        with patch("api.db.cache_manager._cache_metadata", {}) as mock_cache_metadata:
                            with patch("api.db.cache_manager._cache_size_bytes", 0) as mock_cache_size:
                                _initialize_cache()
        ...
        with patch("api.db.cache_manager._cache_size_bytes", 550 * 1024 * 1024):
            with patch("api.db.cache_manager.MAX_CACHE_SIZE_MB", 500):
                with patch("api.db.cache_manager.remove_from_cache") as mock_remove:
                    ...
                    ], any_order=False)  # Order matters, oldest first
...
def test_get_from_cache(mock_cache_content):
    ...
    with patch("api.db.cache_manager._initialize_cache"):
        with patch("api.db.cache_manager._get_cache_metadata", return_value={
            "expiration": int(time.time()) + 3600  # Not expired
        }):
            with patch("api.db.cache_manager._get_cache_path", return_value=Path("/mock/db/cache/test_file.json")):
...
def test_get_from_cache_expired():
    ...
    with patch("api.db.cache_manager._initialize_cache"):
        with patch("api.db.cache_manager._get_cache_metadata", return_value={
            "expiration": int(time.time()) - 3600  # Expired 1 hour ago
        }):
            with patch("api.db.cache_manager.remove_from_cache") as mock_remove:
                result = get_from_cache(url)
                assert result is None
                ...
                with patch("builtins.open", mock_open()) as mock_file:
                    with patch("json.dump") as mock_json_dump:
                        with patch("os.path.getsize", return_value=1024):
                            with patch("api.db.cache_manager._cache_metadata", {}) as mock_cache_metadata:
                                with patch("api.db.cache_manager._cache_size_bytes", 0) as mock_cache_size:
                                    with patch("api.db.cache_manager._cleanup_cache") as mock_cleanup:
                                        add_to_cache(url, content)
                                        
                                        # Check file was opened for writing
                                        mock_file.assert_called_once_with(
                                            Path("/mock/db/cache/test_file.json"), "w", encoding="utf-8"
                                        )
...
def test_remove_from_cache():
    ...
    with patch("api.db.cache_manager._initialize_cache"):
        with patch("api.db.cache_manager._get_cache_metadata", return_value=mock_metadata):
            with patch("api.db.cache_manager._get_cache_path", return_value=Path("/mock/db/cache/test_file.json")):
                with patch("pathlib.Path.exists", return_value=True):
                    with patch("pathlib.Path.unlink") as mock_unlink:
                        with patch("api.db.cache_manager._cache_metadata", {url: mock_metadata}) as mock_cache_metadata:
                            with patch("api.db.cache_manager._cache_size_bytes", 1024) as mock_cache_size:
...
def test_clear_cache():
    ...
    mock_files = [
        Path("/mock/db/cache/file1.json"),
        Path("/mock/db/cache/file2.json")
    ]
    
    with patch("api.db.cache_manager.CACHE_PATH", return_value=Path("/mock/db/cache")):
        with patch("pathlib.Path.glob", return_value=mock_files):
            with patch("pathlib.Path.unlink") as mock_unlink:
                with patch("api.db.cache_manager._cache_metadata", {"url1": {}, "url2": {}}) as mock_cache_metadata:
                    with patch("api.db.cache_manager._cache_size_bytes", 1024) as mock_cache_size:
                        with patch("api.db.cache_manager._cache_initialized", True) as mock_cache_initialized:
                            clear_cache()
...
def test_get_cache_info(mock_cache_content):
    ...
    with patch("api.db.cache_manager._get_cache_metadata", return_value=mock_cache_content["metadata"]):
        with patch("api.db.cache_manager.get_from_cache", return_value=mock_cache_content["content"]):
