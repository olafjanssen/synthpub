...
def test_markdown_to_article(mock_article, mock_article_markdown):
    ...
    with patch(
        "api.db.article_db.yaml.safe_load",
        return_value={
            "id": "test-article-123",
            "title": "Test Article",
            "topic_id": "test-topic-456",
...
def test_get_article(mock_article, mock_article_markdown):
    ...
    with patch("api.db.article_db.DB_PATH", return_value=Path("/mock/db/articles")):
        with patch(
            "pathlib.Path.glob",
            return_value=[Path("/mock/db/articles/test-article-123.md")],
        ):
            with patch("builtins.open", mock_open(read_data=mock_article_markdown)):
                with patch(
                    "api.db.article_db.markdown_to_article", return_value=mock_article
                ):
...
def test_list_articles(mock_article):
    ...
    with patch("api.db.article_db.ensure_db_exists"):
        with patch("api.db.article_db.DB_PATH", return_value=Path("/mock/db/articles")):
            with patch(
                "pathlib.Path.glob",
                return_value=[Path("/mock/db/articles/test-article-123_v1.md")],
            ):
                with patch("builtins.open", mock_open()):
                    with patch(
                        "api.db.article_db.markdown_to_article",
